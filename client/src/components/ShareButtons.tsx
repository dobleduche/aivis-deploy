...
export default function ShareButtons({ url, score, title, analyzedAt, scanCount, auditId }: ShareButtonsProps) {
  ...
  const shareText = (() => {
    if (score >= 90) return `${displayDomain} scored ${score}/100 on AI visibility. This site is citation-ready across ChatGPT, Perplexity, Claude, and Google AI.`;
    if (score >= 75) return `${displayDomain} scored ${score}/100 — strong AI visibility with room to tighten structured data and citation signals.`;
    if (score >= 60) return `${displayDomain} pulled a ${score}/100 on AI visibility. Decent foundation but missing key signals that AI models look for.`;
    if (score >= 40) return `${displayDomain} got ${score}/100 for AI visibility. Most answer engines are overlooking this site — fixable with structured markup.`;
    return `${displayDomain} scored ${score}/100 on AI visibility. Answer engines can barely read this site — real fixes needed.`;
  })();
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <button
        ...
        title="Share Link"
      >
        <Share2 className="w-3.5 h-3.5 text-amber-300" /> Share Link
      </button>
    </div>
  );
}
