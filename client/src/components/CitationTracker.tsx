...
function WebSearchCard({ data }: { data: WebSearchPresenceResult }) {
  ...
  const accentBg = isInstant ? 'bg-emerald-500/10' : isBing ? 'bg-blue-500/10' : isBrave ? 'bg-orange-500/10' : 'bg-cyan-500/10';
  const accentIcon = isInstant ? 'text-emerald-400' : isBing ? 'text-blue-400' : isBrave ? 'text-orange-400' : 'text-cyan-400';
  const accentText = isInstant ? 'text-emerald-300/90' : isBing ? 'text-blue-300/90' : isBrave ? 'text-orange-300/90' : 'text-cyan-300/90';
  const accentBadgeBg = isInstant ? 'bg-emerald-500/20' : isBing ? 'bg-blue-500/20' : isBrave ? 'bg-orange-500/20' : 'bg-cyan-500/20';
  const accentBadgeText = isInstant ? 'text-emerald-400' : isBing ? 'text-blue-400' : isBrave ? 'text-orange-400' : 'text-cyan-400';
  const label = isInstant ? 'DuckDuckGo Instant' : isBing ? 'Bing' : isBrave ? 'Brave' : 'DuckDuckGo Web';
...
function QueryResults({ query, results, webSearch, bingSearch, ddgSearch, braveSearch }: QueryResultsProps) {
  const [expanded, setExpanded] = useState(false);
      ...
      {groupedResults.length > 0 && (
        <div className="space-y-3">
          {groupedResults.map((group, i) => (
            <QueryResults key={i} query={group.query} results={group.results} webSearch={webSearchByQuery?.[group.query]} bingSearch={bingSearchByQuery?.[group.query]} ddgSearch={ddgSearchByQuery?.[group.query]} braveSearch={braveSearchByQuery?.[group.query]} />
          ))}
        </div>
      )}
